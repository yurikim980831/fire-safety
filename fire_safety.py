...
with st.expander("📍 **소방 수신반 및 상수도 위치 도면 보기 (클릭하여 펼치기)**", expanded=False):
    ...
    if fp_img_path:
        # PC 모니터에서 너무 커지지 않도록 이미지 너비 조정 및 중앙 정렬
        img_col1, img_col2, img_col3 = st.columns([1, 4, 1])
        with img_col2:
            st.image(fp_img_path, caption="사업장 내 수신반(주제어동 3층 CCR) 및 상수도(스팀터빈동 주출입구 앞) 배치 도면", width=750)
    else:
...
with st.expander("📍 **사내 AED(자동심장충격기) 설치 위치 및 안내문 보기 (클릭하여 펼치기)**", expanded=False):
    ...
    if aed_img_path:
        ...
        with aed_col2:
            st.image(aed_img_path, caption="사내 AED 설치 장소 (관리동 3층 E/V 앞, 주제어동 CCR 앞) 및 주의사항", width=700)
    else:
    ...
    for file_candidates, caption in images_info:
        ...
        for fname in file_candidates:
            ...
            if os.path.exists(path):
                ...
                break
        if found_path:
            # PC 모니터에서 세로 문서 이미지가 지나치게 거대해지는 현상 방지
            s_col1, s_col2, s_col3 = st.columns([1, 4, 1])
            with s_col2:
                st.image(found_path, caption=caption, width=680)
            st.markdown("---")
