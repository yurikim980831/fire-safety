...
with cpr_col:
    with st.container(border=True):
        st.markdown("#### 🫀 심폐소생술(CPR) 방법")
        st.markdown("""
        1. **의식 및 호흡 확인:** 어깨를 가볍게 두드리며 반응 및 정상 호흡 여부를 확인합니다.
        2. **119 신고 및 AED 요청:** 주변 사람을 특정하여 **119 신고**와 **AED(자동심장충격기)**를 가져오도록 지시합니다.
        3. **가슴압박 30회 실시:**
           * 깍지 낀 두 손으로 가슴 중앙(양 젖꼭지 연결선 한가운데)을 깍지 낀 채 누릅니다.
           * **압박 깊이:** 약 5cm 깊이 / **속도:** 분당 100~120회
        4. **인공호흡 2회 실시:** 기도를 확보한 후 환자의 코를 막고 가슴이 부풀어 오르도록 2회 숨을 넣습니다.
        5. **무한 반복:** 119 구급대가 도착할 때까지 **가슴압박 30회 + 인공호흡 2회**를 계속 반복합니다.
        """)

with aed_col:
    with st.container(border=True):
        st.markdown("#### ⚡ 자동심장충격기(AED) 사용법")
        st.markdown("""
        1. **전원 켜기:** AED를 환자 옆에 두고 전원 버튼을 누릅니다. (음성 지시 시작)
        2. **패드 부착:** 
           * **패드 1:** 오른쪽 빗장뼈(쇄골) 아래
           * **패드 2:** 왼쪽 젖꼭지 아래 겨드랑이선
        3. **심장리듬 분석:** "분석 중..." 음성이 나오면 **환자에게서 모두 물러납니다.**
        4. **제세동(전기충격):** 제세동이 필요하다면 충전 후 버튼이 깜빡입니다. **모두 물러선 것을 확인 후 버튼을 누릅니다.**
        5. **즉시 CPR 재개:** 전기충격 직후 즉시 가슴압박을 다시 시작합니다.
        """)

# AED 사내 비치장소 펼치기 (크기 축소 및 중앙 정렬)
