...
def fetch_safety_news():
    ...
    target_urls = [
        ...
        "https://www.mosafe.go.kr/rss/news.xml"
    ]
                ...
                for item in root.findall(".//item"):
                    ...
                    if title:
                        articles.append({
                            ...
                            "link": link,
                            "date": pub_date,
                            "content": description if description else "상세 내용을 확인하려면 원문 링크를 클릭하세요."
                        })
                    if len(articles) >= 5:
                        break
            if articles:
...
if releases:
    ...
    st.info("💡 현재 최신 보도자료를 불러오는 중입니다. 아래 링크를 통해 직접 확인하실 수도 있습니다.")
    st.markdown("👉 [🔗 소방청 보도자료 게시판 바로가기](https://www.nfa.go.kr/nfa/news/pressrelease/press/)")
