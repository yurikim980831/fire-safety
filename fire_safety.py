...
# -------------------------------------------------------------
# 커스텀 표 생성 함수 (비상연락망 및 시나리오용 - 폰트 & 행높이 축소 적용)
# -------------------------------------------------------------
...
# -------------------------------------------------------------
# 소방청 보도자료 수집 함수 (상세 페이지 direct link 매핑)
# -------------------------------------------------------------
...
def fetch_safety_news():
    ...
    # 크롤링 제한 또는 오류 시 최신 소방청 실제 기사 링크 대체
    if not articles:
        articles = [
            {
                "title": "재난대응 역량 강화를 위한 소방·경찰·해양경찰 간부후보생 합동 교육훈련",
                "link": "https://www.nfa.go.kr/nfa/news/pressrelease/press/"
            },
            {
                "title": "소방청, 필승교 수위 상승에 중앙긴급구조통제단 가동",
                "link": "https://www.nfa.go.kr/nfa/news/pressrelease/press/"
            },
            {
                "title": "소방청, 인천 서해구 쿠팡32물류센터 화재 「중앙화재 합동조사단」 운영",
                "link": "https://www.nfa.go.kr/nfa/news/pressrelease/press/"
            },
            {
                "title": "벌 쏘임 사고 집중 시기… 소방청, 여름철 야외활동 안전수칙 준수 당부",
                "link": "https://www.nfa.go.kr/nfa/news/pressrelease/press/"
            },
            {
                "title": "대한민국 밧줄(로프)구조 세계적 경쟁력 강화…국제 기술교류 합동훈련 개최",
                "link": "https://www.nfa.go.kr/nfa/news/pressrelease/press/"
            }
        ]
...
# =============================================================
# [섹션 3] 비상 대피소 안내 (1차 대피소 위치 변경 적용)
# =============================================================
...
# =============================================================
# [섹션 5] 주요 소방시설 (수신반 & 상수도) 위치 안내 (폰트 크기 축소 반영)
# =============================================================
st.subheader("💧 주요 소방시설 (수신반 & 상수도) 위치")

col_rec, col_water = st.columns(2)

# 칸 내부 설치위치 및 주요역할 폰트 축소를 위한 스타일 정의
st.markdown("""
    <style>
    .facility-box {
        border: 1px solid #e2e8f0;
...
※ 금년도 화재예방안전진단 수검으로 인한 교육·훈련 면제로 11월 합동 교육·훈련이 아닌 자체 실시를 통해 약소화 진행 예정
</div>
""", unsafe_allow_html=True)
...
# =============================================================
# [섹션 9] 최신 소방청 보도자료 (날짜 제거 및 소방청 Direct URL 링크 연결)
# =============================================================
st.subheader("📰 최신 소방청 보도자료")

releases = fetch_safety_news()
for rel in releases:
    with st.container(border=True):
        st.markdown(f"**📌 {rel['title']}**")
        st.markdown(f"🔗 [해당 소방청 보도자료 원문 바로가기]({rel['link']})")
